...
export {
	...
	## :zeek:see:`LogAsciiBatch::gzip_file_extension`.
	##
	## This option is also available as a per-filter ``$config`` option.
	const gzip_level = 0 &redef;

	## Define the file extension used when compressing log files when
	## they are created with the :zeek:see:`LogAsciiBatch::gzip_level` option.
	##
	## This option is also available as a per-filter ``$config`` option.
...
redef Log::default_rotation_postprocessors += { [Log::WRITER_ASCII_BATCH] = default_rotation_postprocessor_func };
